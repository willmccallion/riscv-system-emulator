...
//!
//! This module provides constants for all RISC-V CSRs and implements
//! the CSR register file with read/write operations. CSRs control
//! processor state, interrupt handling, memory management, and
...
///
/// Contains all machine-level and supervisor-level CSRs that control
/// processor state, interrupt handling, memory management, and performance
/// counters. This structure maintains the complete CSR state for the processor.
...
impl Csrs {
    ...
    /// # Returns
    ///
    /// The 64-bit value stored in the specified CSR, or 0 if the
    /// address is not recognized or not implemented.
    ...
    /// # Note
    ///
    /// Some CSRs have special write behavior (e.g., SATP mode validation).
    /// Writes to read-only or unimplemented CSRs are silently ignored.
